...
{
	include("cumchucnang/bien/bien.php");
	$sql = "select * from menu_ngang where thuoc_menu='' order by id";
	$query = mysql_query($sql);
	while($rows = mysql_fetch_array($query))
	{
		$chuoi_menu = $chuoi_menu." ( select * from menu_ngang where id = '$rows[id]') union";
...
$sql_tsd = "select count(*) from menu_ngang";
