...
<?php
	...
	function dem_ketqua_tk($tu_khoa)
	{
		$chuoi="select count(*) from dulieu where tieu_de like '%$tu_khoa%' ";
		//thongbao("$chuoi");
		$r_sql=mysql_fetch_row(mysql_query($chuoi));
		return $r_sql[0];
	}
	
	$mang_tu_khoa=explode(" ",trim($tu_khoa));
	$mang_tu_khoa_1=mang_bo_khoang_trang_co_sap_xep($mang_tu_khoa);
	//print_r($mang_tu_khoa_1);
	$chuoi_or=="";
	for($i=0;$i<count($mang_tu_khoa_1);$i++)
	{
		$chuoi_or=$chuoi_or."tieu_de like '%$mang_tu_khoa_1[$i]%' or ";
	}
	$chuoi_or=substr($chuoi_or,0,-3);
	//echo $chuoi_or;

	$dem=dem_ketqua_tk($tu_khoa);
	$st=ceil($dem/$so_gioi_han);
	$chuoi_tk_sql="select * from dulieu where tieu_de like '%$tu_khoa%' order by id desc limit $vtbd,$so_gioi_han";
...
?>
<div class="headboxpro"><a href='javascript:voild(0)' >Kết quả tìm kiếm</a>
